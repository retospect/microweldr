...
def main():
    ...
    try:
        ...
        try:
            ...
            print(
                f"   ✓ Available storage: {len(storage.get('storage_list', []))} devices"
            )
            for store in storage.get("storage_list", []):
                print(
                    f"      - {store.get('name', 'Unknown')}: {store.get('free_space', 0)} bytes free"
                )
        except PrusaLinkError as e:
        ...
        print(
            "Files will be uploaded and started automatically with your current config."
        )
        print("\nUsage:")
        print(
            "  svg-welder input.svg --submit-to-printer  # Will auto-start based on config"
        )
        print(
            "  svg-welder input.svg --submit-to-printer --auto-start-print  # Force auto-start"
        )
