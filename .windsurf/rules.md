...
Type check:          uv run mypy microweldr tests
Run a script:        uv run python script.py
Run the CLI:         uv run microweldr --help
Full check:          uv run ruff check . && uv run ruff format --check . && uv run mypy microweldr tests && uv run pytest
