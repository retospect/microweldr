...
class TestSafetyValidation:
    ...
    @given(
        st.floats(
            min_value=120.1, max_value=1000, allow_nan=False, allow_infinity=False
        )
    )
    ...
    @given(
        st.floats(min_value=0.5001, max_value=10, allow_nan=False, allow_infinity=False)
    )
    ...
    @given(
        st.floats(min_value=5.001, max_value=100, allow_nan=False, allow_infinity=False)
    )
...
class TestWeldPointGeneration:
    ...
    @given(
        ...
        custom_weld_height=st.one_of(
            st.none(), st.floats(min_value=0.001, max_value=0.5)
        ),
    )
    ...
    @given(
        points=st.lists(
            st.builds(
                WeldPoint,
                x=st.floats(
                    min_value=0, max_value=250, allow_nan=False, allow_infinity=False
                ),
                y=st.floats(
                    min_value=0, max_value=220, allow_nan=False, allow_infinity=False
                ),
                weld_type=st.sampled_from(
                    [WeldType.NORMAL.value, WeldType.LIGHT.value]
                ),
                custom_temp=st.one_of(
                    st.none(), st.floats(min_value=50, max_value=120)
                ),
                custom_weld_time=st.one_of(
    ...
    def test_weld_path_creation_and_optimization(self, points, weld_type, name):
        ...
        for opt_point in optimized_path.points:
            assert any(
                abs(opt_point.x - orig_point.x) < 0.001
                and abs(opt_point.y - orig_point.y) < 0.001
...
class TestConfigurationValidation:
    ...
    @given(
        bed_temp=st.floats(
            min_value=0, max_value=80, allow_nan=False, allow_infinity=False
        ),
        nozzle_temp=st.floats(
            min_value=50, max_value=120, allow_nan=False, allow_infinity=False
        ),
        weld_temp=st.floats(
            min_value=50, max_value=120, allow_nan=False, allow_infinity=False
        ),
        weld_height=st.floats(
        ...
        travel_speed=st.floats(
            min_value=100, max_value=3000, allow_nan=False, allow_infinity=False
        ),
        z_speed=st.floats(
            min_value=50, max_value=1000, allow_nan=False, allow_infinity=False
        ),
    )
    def test_valid_configuration_always_passes(
        ...
        config = {
            ...
            "movement": {"travel_speed": travel_speed, "z_speed": z_speed},
        }
...
WeldOperationStateMachine.TestCase.settings = settings(
...
TestWeldOperationStateMachine = WeldOperationStateMachine.TestCase
