...
from microweldr.core.resource_management import safe_gcode_generation, TemporaryFileManager
...
from microweldr.validation.validators import SVGValidator, GCodeValidator, AnimationValidator
...
"""
    ...
    def test_svg_to_gcode_complete_workflow(self, test_config, test_svg):
        ...
        with TemporaryFileManager(prefix="workflow_test_") as temp_manager:
            ...
            with safe_gcode_generation(gcode_path) as temp_gcode_path:
                ...
                generator.generate(weld_paths, str(temp_gcode_path))
            
            # Step 4: Validate generated G-code
            validator = GCodeValidator()
            result = validator.validate(str(gcode_path))
            assert result.is_valid or len(result.warnings) == 0  # Should be valid or have acceptable warnings
    ...
    def test_svg_to_animation_workflow(self, test_config, test_svg):
        ...
        with TemporaryFileManager(prefix="animation_test_") as temp_manager:
            ...
            assert 'animate' in animation_content or 'animateTransform' in animation_content
    ...
    def test_printer_communication_workflow(self, m, test_config, test_svg, secrets_file):
        """Test workflow with printer communication."""
        # Mock PrusaLink responses
        m.get("http://192.168.1.100/api/printer", json={
            "printer": {
                "state": "Operational",
        ...
        m.post("http://192.168.1.100/api/files/local", json={
    ...
    def test_error_recovery_workflow(self, test_config, secrets_file):
        ...
        with TemporaryFileManager(prefix="error_test_") as temp_manager:
            ...
            with requests_mock.Mocker() as m:
                ...
                status = client.get_status()
                assert status.get("fallback") is True or status.get("state") == "Disconnected"
    ...
    def test_concurrent_workflow(self, test_config, test_svg):
        ...
        def worker_process_svg(svg_path, worker_id):
            ...
            try:
                ...
                with TemporaryFileManager(prefix=f"worker_{worker_id}_") as temp_manager:
                    gcode_path = temp_manager.create_temp_file(suffix=".gcode")
                    generator = GCodeGenerator(test_config)
                    generator.generate(weld_paths, str(gcode_path))
...
<svg width="1000" height="1000" xmlns="http://www.w3.org/2000/svg">'''
        ...
        for i in range(1000):
            ...
            x2, y2 = x1 + 5, y1
            large_svg_content += f'<line x1="{x1}" y1="{y1}" x2="{x2}" y2="{y2}" stroke="black" />\n'
        ...
        with TemporaryFileManager(prefix="memory_test_") as temp_manager:
            ...
            assert gcode_path.stat().st_size > 1000  # Should generate substantial G-code
