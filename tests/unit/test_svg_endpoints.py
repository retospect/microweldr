  ...
  <rect x="10" y="20" width="60" height="40" fill="none" stroke="black" stroke-width="1"/>
        ...
        try:
            ...
            for expected_x, expected_y in expected_corners:
                found_corner = False
                for point in path.points:
                    if abs(point.x - expected_x) < 0.1 and abs(point.y - expected_y) < 0.1:
                        found_corner = True
                        break
                assert found_corner, f"Corner ({expected_x}, {expected_y}) not found in rectangle points"
            ...
            for expected_x, expected_y in expected_points:
                ...
                for point in path.points:
                    if abs(point.x - expected_x) < 0.1 and abs(point.y - expected_y) < 0.1:
                        found_point = True
                        break
                assert found_point, f"Expected point ({expected_x}, {expected_y}) not found in path"
  ...
  <circle cx="110" cy="30" r="20" fill="none" stroke="red" stroke-width="1"/>
        ...
        try:
            ...
            for i, path in enumerate(weld_paths):
                ...
                assert abs(first_point.x - last_point.x) < 0.001, f"Circle {i} not closed in X"
                assert abs(first_point.y - last_point.y) < 0.001, f"Circle {i} not closed in Y"
                assert first_point.weld_type == last_point.weld_type, f"Circle {i} weld types don't match"
  ...
  <path d="M 100 100 L 140 100 L 140 130 Z" fill="none" stroke="black"/>
        ...
        try:
            ...
            for path in weld_paths:
                ...
                if abs(first_point.x - last_point.x) < 0.001 and abs(first_point.y - last_point.y) < 0.001:
                    closed_paths.append(path)
                else:
                    open_paths.append(path)
            
            # We should have 3 closed paths (circle, rectangle, path with Z) and 1 open (line)
            assert len(closed_paths) == 3, f"Expected 3 closed paths, got {len(closed_paths)}"
            ...
            for i, path in enumerate(closed_paths):
                first_point = path.points[0]
                last_point = path.points[-1]
                assert abs(first_point.x - last_point.x) < 0.001, f"Closed path {i} not closed in X"
                assert abs(first_point.y - last_point.y) < 0.001, f"Closed path {i} not closed in Y"
...
<svg width="100" height="100" xmlns="http://www.w3.org/2000/svg">
  <circle cx="50" cy="50" r="20" fill="none" stroke="black" stroke-width="1"/>
        ...
        try:
            ...
            if len(angles) > 2:  # Need at least 3 points for meaningful distribution check
                ...
                for i in range(1, len(angles)):
                    ...
                    if abs(angle_diff - expected_angle_step) < expected_angle_step * 2.0:
                        good_angles += 1
                
                # Most angles should be reasonably distributed
                assert good_angles >= len(angles) * 0.5, f"Only {good_angles}/{len(angles)-1} angles well distributed"
