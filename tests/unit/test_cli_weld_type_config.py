...
@pytest.fixture
def mock_config():
    """Create a mock config with specific odd test values."""
    ...
    def config_get(section, key, default=None):
        config_values = {
            ...
            # Test-specific odd values
            ("normal_welds", "weld_height"): 0.123,
            ("normal_welds", "weld_time"): 0.456,
            ("normal_welds", "weld_temperature"): 165,
            ("normal_welds", "dot_spacing"): 0.8,
            ("frangible_welds", "weld_height"): 0.234,
            ("frangible_welds", "weld_time"): 0.789,
            ("frangible_welds", "weld_temperature"): 170,
...
def custom_config_file(tmp_path):
    """Create a config file with specific odd test values (for file-based tests)."""
    config_content = """
[printer]
bed_size_x = 250.0
bed_size_y = 220.0
...
class TestWeldTypeConfigFlow:
    ...
    def test_normal_weld_config_to_gcode(self, simple_dxf_file, mock_config, tmp_path):
        """Test that normal weld config values flow correctly to G-code output."""
        points = process_weld_file(
            str(simple_dxf_file), mock_config, is_frangible=False
        )
        ...
        success = generate_gcode(points, str(output_gcode), mock_config, Args())
        assert success
        assert output_gcode.exists()

        gcode_content = output_gcode.read_text()

        # Verify our custom test values made it through
    ...
    def test_frangible_weld_config_to_gcode(
        self, simple_dxf_file, mock_config, tmp_path
    ):
        """Test that frangible weld config values flow correctly to G-code output."""
        points = process_weld_file(str(simple_dxf_file), mock_config, is_frangible=True)
    ...
    def test_mixed_weld_types_in_same_gcode(
        ...
        frangible_points = process_weld_file(
            str(simple_dxf_file), mock_config, is_frangible=True
        )

        all_points = normal_points + frangible_points

        normal_count = sum(1 for p in all_points if p["weld_type"] == "normal")
        frangible_count = sum(1 for p in all_points if p["weld_type"] == "frangible")
        ...
        success = generate_gcode(all_points, str(output_gcode), mock_config, Args())
        assert success
        assert output_gcode.exists()

        gcode_content = output_gcode.read_text()

        # Verify both weld types use their custom config values
        assert "G1 Z0.123 F3000 ; Lower to weld height" in gcode_content
        assert "G4 P456 ; Weld for 0.456s" in gcode_content
        assert "G1 Z0.234 F3000 ; Lower to frangible weld height" in gcode_content
        assert "G4 P789 ; Frangible weld for 0.789s" in gcode_content
    ...
    def test_config_values_precision(self, simple_dxf_file, mock_config, tmp_path):
        """Test that odd decimal values are preserved with correct precision in G-code."""
        # Verify mock config has our odd test values
        assert mock_config.get("normal_welds", "weld_height") == 0.123
        assert mock_config.get("normal_welds", "weld_time") == 0.456
        assert mock_config.get("frangible_welds", "weld_height") == 0.234
        assert mock_config.get("frangible_welds", "weld_time") == 0.789
        ...
        generate_gcode(normal_points, str(output_gcode), mock_config, Args())
        gcode_content = output_gcode.read_text()

        # Verify precision is maintained in G-code output
