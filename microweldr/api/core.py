...
class WeldJob:
    ...
    def __init__(self, svg_path: Union[str, Path], config: Optional[Config] = None):
        ...
        self._temp_manager = TemporaryFileManager(prefix=f"weld_job_{self.svg_path.stem}_")
            ...
            try:
                ...
                if not svg_result.is_valid:
                    self.validation_result = ValidationResult(
                        False, 
                        errors=[f"SVG validation failed: {svg_result.message}"]
                    )
                    return self.validation_result
                ...
                # Safety validation
                warnings, errors = validate_weld_operation(self.weld_paths, self.config.config)
    ...
    def _parse_svg(self, use_cache: bool = True) -> None:
        ...
        with LogContext("svg_parsing"):
            ...
            logger.info(f"Parsed {len(self.weld_paths)} weld paths from {self.svg_path}")
    ...
    def generate_gcode(
        ...
        with LogContext("gcode_generation"):
            ...
            with safe_gcode_generation(output_path, backup=True) as temp_gcode_path:
                ...
                generator.generate(self.weld_paths, str(temp_gcode_path), skip_bed_leveling=skip_bed_leveling)
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'validation_status': str(self.validation_result) if self.validation_result else 'Not validated'
        }
...
class MicroWeldr:
    ...
    def __init__(self, config_path: Optional[Union[str, Path]] = None, log_level: str = "INFO"):
                ...
                with self.create_job(svg_path) as job:
                    ...
                    if not validation.is_valid:
                        if continue_on_error:
                            ...
                            raise SafetyError(f"Validation failed for {svg_path}: {validation.errors}")
                    
                    # Determine output paths
                    if output_dir:
                        gcode_path = output_dir / svg_path.with_suffix('.gcode').name
                        animation_path = output_dir / svg_path.with_suffix('_animation.svg').name if generate_animations else None
                    else:
                        gcode_path = svg_path.with_suffix('.gcode')
                        animation_path = svg_path.with_suffix('_animation.svg') if generate_animations else None
                    ...
                    results['successful'].append({
                        ...
                        'animation': str(animation_path) if animation_path else None,
