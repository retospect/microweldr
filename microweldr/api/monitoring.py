...
from ..core.health_checks import HealthChecker, quick_health_check, generate_health_report
...
class HealthStatus:
    ...
    def get_failed_checks(self) -> List[str]:
        ...
        """
        return [name for name, data in self.checks.items() if data.get('status') == 'error']
    ...
    def get_degraded_checks(self) -> List[str]:
        ...
        """
        return [name for name, data in self.checks.items() if data.get('status') == 'warning']
...
class SystemMonitor:
    ...
    def get_monitoring_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'last_check_time': self._last_health_check.timestamp if self._last_health_check else None,
            'last_status': str(self._last_health_check) if self._last_health_check else None
        }
...
def create_monitoring_callback(log_level: str = "INFO") -> callable:
    ...
    def callback(status: HealthStatus):
        ...
        if status.is_unhealthy:
            logger.error(f"System Health: {status.overall} ({status.error_count} errors)")
            for error in status.errors[:3]:  # Log first 3 errors
                logger.error(f"  - {error}")
        elif status.is_degraded:
            logger.warning(f"System Health: {status.overall} ({status.warning_count} warnings)")
        else:
            if log_level == "DEBUG":
