    ...
    def execute(self, context: Dict[str, Any]) -> Any:
        ...
        for attempt in range(self.retry_count + 1):
            ...
            try:
                ...
                logger.info(f"Step '{self.name}' completed successfully in {self.duration:.2f}s")
                return self.result
                
            except Exception as e:
                self.error = e
                logger.warning(f"Step '{self.name}' failed (attempt {self.attempts}): {e}")
                ...
                if attempt < self.retry_count:
                    ...
                    if self.required:
                        logger.error(f"Required step '{self.name}' failed after {self.attempts} attempts")
                        raise
                    else:
                        logger.warning(f"Optional step '{self.name}' failed, continuing workflow")
                        return None
...
class WorkflowManager:
    ...
    def add_gcode_generation_step(
        ...
        def gcode_action(context):
            ...
            gcode_path = job.generate_gcode(output_path, skip_bed_leveling=skip_bed_leveling)
    ...
    def add_print_monitoring_step(
        ...
        def monitoring_action(context):
            def status_callback(status: PrinterStatus):
                logger.info(f"Print status: {status.state} ({status.job_progress:.1f}%)")
    ...
    def execute(self, stop_on_failure: bool = True, progress_callback: Optional[Callable] = None) -> Dict[str, Any]:
        ...
        try:
            with progress_context(len(self.steps), f"Executing {self.name}") as progress:
            ...
            logger.info(f"Workflow '{self.name}' completed successfully in {results['duration']:.2f}s")
    ...
    def get_results(self) -> Dict[str, Any]:
        ...
        """
        completed_steps = [s for s in self.steps if s.status == WorkflowStepStatus.COMPLETED]
        failed_steps = [s for s in self.steps if s.status == WorkflowStepStatus.FAILED]
        skipped_steps = [s for s in self.steps if s.status == WorkflowStepStatus.SKIPPED]
        ...
        return {
            ...
            'success_rate': len(completed_steps) / len(self.steps) * 100 if self.steps else 0,
...
def create_batch_workflow(
    ...
    for i, svg_file in enumerate(svg_files):
        ...
        workflow.add_animation_generation_step(animation_path, name=f"animation_{i}", required=False)
