"""Data models for structured data in MicroWeldr."""
...
@dataclass
class CADEntity:
    """Base class for CAD entities (lines, arcs, circles)."""

    layer: str
    entity_type: str
    is_construction: bool = field(default=False)

    def __post_init__(self):
        """Mark construction entities."""
        # Common construction layer patterns
        construction_patterns = ["construction", "const", "guide", "reference", "ref"]
        self.is_construction = any(
            pattern in self.layer.lower() for pattern in construction_patterns
        )
