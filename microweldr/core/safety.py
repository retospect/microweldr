...
class SafetyValidator:
    ...
    def validate_temperature(self, temp: float, param_name: str = "temperature") -> None:
    ...
    def validate_weld_height(self, height: float, param_name: str = "weld_height") -> None:
    ...
    def validate_weld_path(self, path: WeldPath) -> None:
        ...
        if path.custom_weld_height is not None:
            self.validate_weld_height(path.custom_weld_height, "path custom_weld_height")
        ...
        logger.debug(f"Weld path validation passed: {path.name} ({len(path.points)} points)")
    ...
    def validate_config(self, config: Dict) -> Tuple[List[str], List[str]]:
        ...
        try:
            ...
            if "nozzle_temperature" in temps:
                self.validate_temperature(temps["nozzle_temperature"], "nozzle_temperature")
            ...
            for weld_type in ["normal_welds", "light_welds"]:
                if weld_type in config:
                    ...
                    if "weld_temperature" in weld_config:
                        self.validate_temperature(weld_config["weld_temperature"], f"{weld_type}.weld_temperature")
                        
                    if "weld_height" in weld_config:
                        self.validate_weld_height(weld_config["weld_height"], f"{weld_type}.weld_height")
                        
                    if "weld_time" in weld_config:
                        self.validate_weld_time(weld_config["weld_time"], f"{weld_type}.weld_time")
            
            # Validate movement settings
            movement = config.get("movement", {})
            if "travel_speed" in movement:
                self.validate_speed(movement["travel_speed"], "travel_speed", self.MAX_TRAVEL_SPEED)
...
def validate_weld_operation(weld_paths: List[WeldPath], config: Dict) -> Tuple[List[str], List[str]]:
