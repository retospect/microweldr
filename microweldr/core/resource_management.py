    ...
    def __enter__(self):
        ...
        with self._lock:
            try:
                ...
                if self.backup_original and self.file_path.exists() and 'w' in self.mode:
...
class TemporaryFileManager:
    ...
    def __init__(self, suffix: str = '', prefix: str = 'microweldr_', dir: Optional[str] = None):
    ...
    def _get_resource(self, factory_func, *args, **kwargs):
        ...
        with self._lock:
            if self.resources:
                ...
                logger.debug(f"Reused resource from pool (pool size: {len(self.resources)})")
            else:
    ...
    def _return_resource(self, resource_id: int, resource: Any):
        ...
        with self._lock:
            ...
            if len(self.resources) < self.max_size:
                ...
                logger.debug(f"Returned resource to pool (pool size: {len(self.resources)})")
            else:
...
@contextmanager
def safe_gcode_generation(output_path: Union[str, Path], backup: bool = True) -> Generator[Path, None, None]:
...
@contextmanager
def managed_printer_operation(operation_name: str) -> Generator[Dict[str, Any], None, None]:
