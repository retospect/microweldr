...
class SecretsValidator:
    ...
    WEAK_PASSWORDS = {
        'password', '123456', 'admin', 'root', 'user', 'guest', 'test',
        'default', 'changeme', 'welcome', 'qwerty', 'abc123', 'letmein',
    ...
    def validate_password_strength(self, password: str, username: str = None) -> Tuple[bool, List[str]]:
        ...
        if char_types < 2:
            issues.append("Password should contain at least 2 different character types (lowercase, uppercase, digits, special characters)")
    ...
    def validate_ip_address(self, ip_str: str) -> Tuple[bool, Optional[str]]:
        ...
        try:
            ...
            if not ip.is_private and not ip.is_loopback:
                logger.warning(f"Using public IP address: {ip_str} - ensure this is intentional")
    ...
    def validate_secrets_file(self, secrets_path: str) -> Tuple[List[str], List[str]]:
        ...
        try:
            ...
            if file_stat.st_mode & stat.S_IROTH:
                self.warnings.append(f"Secrets file is readable by others: {secrets_path}")
            
            # Check if file is writable by others
            if file_stat.st_mode & stat.S_IWOTH:
                self.errors.append(f"Secrets file is writable by others: {secrets_path}")
    ...
    def _validate_prusalink_config(self, prusalink_config: Dict) -> None:
        ...
        if not password and not api_key:
            self.errors.append("PrusaLink configuration must include either 'password' or 'api_key'")
        
        # Check for default credentials
        if (username, password) in self.DEFAULT_CREDENTIALS:
            self.errors.append(f"Default credentials detected: {username}/{password} - please change them")
        ...
        if 'timeout' in prusalink_config:
            ...
            if not isinstance(timeout, (int, float)) or timeout <= 0:
                ...
                self.warnings.append(f"Very short timeout: {timeout}s - may cause connection issues")
            elif timeout > 300:
                self.warnings.append(f"Very long timeout: {timeout}s - may cause hanging")
            ...
            if path.is_absolute():
                for dangerous in dangerous_paths:
                    if str(resolved_path).startswith(dangerous):
                        issues.append(f"Path accesses potentially dangerous directory: {dangerous}")
...
def validate_secrets_interactive(secrets_path: str) -> bool:
    ...
    if warnings:
        try:
            response = input("\n⚠️  Security warnings found. Continue anyway? (y/N): ").strip().lower()
