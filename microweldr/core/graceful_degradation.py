...
from ..prusalink.exceptions import PrusaLinkError, PrusaLinkConnectionError, PrusaLinkAuthError
        ...
        def wrapper(*args, **kwargs):
            ...
            for attempt in range(max_retries + 1):
                try:
                    ...
                    if failure_count > 0:
                        logger.info(f"Operation recovered after {failure_count} failures")
                    return result
                    ...
                    if failure_count >= escalate_after:
                        logger.error(f"Operation failed after {max_retries + 1} attempts, using fallback")
...
class ResilientPrusaLinkClient:
    ...
    def upload_file(self, file_path: str, filename: str = None, auto_start: bool = False) -> Dict:
