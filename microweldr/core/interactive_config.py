...
class InteractiveConfigValidator:
            ...
            if "exceeds maximum safe limit" in error:
                ...
                if "bed_temperature" in error and "80" in error:
                    ...
                    if "normal_welds" in error:
                        fixed_config.setdefault("normal_welds", {})["weld_temperature"] = 120
                        fixes_applied.append("Set normal_welds.weld_temperature to safe maximum (120°C)")
                    elif "light_welds" in error:
                        fixed_config.setdefault("light_welds", {})["weld_temperature"] = 120
                        fixes_applied.append("Set light_welds.weld_temperature to safe maximum (120°C)")
                
                elif "weld_height" in error and "0.5" in error:
                    if "normal_welds" in error:
                        fixed_config.setdefault("normal_welds", {})["weld_height"] = 0.5
                        fixes_applied.append("Set normal_welds.weld_height to safe maximum (0.5mm)")
                    elif "light_welds" in error:
                        fixed_config.setdefault("light_welds", {})["weld_height"] = 0.5
                        fixes_applied.append("Set light_welds.weld_height to safe maximum (0.5mm)")
                
                elif "weld_time" in error and "5.0" in error:
                    if "normal_welds" in error:
                        fixed_config.setdefault("normal_welds", {})["weld_time"] = 5.0
                        fixes_applied.append("Set normal_welds.weld_time to safe maximum (5.0s)")
                    elif "light_welds" in error:
                        fixed_config.setdefault("light_welds", {})["weld_time"] = 5.0
                        fixes_applied.append("Set light_welds.weld_time to safe maximum (5.0s)")
                
                elif "travel_speed" in error and "3000" in error:
                    fixed_config.setdefault("movement", {})["travel_speed"] = 3000
                    fixes_applied.append("Set travel_speed to safe maximum (3000 mm/min)")
    ...
    def _fix_warnings_interactive(self, config_path: Path, config: Dict, warnings: List[str]) -> None:
            ...
            if "below recommended minimum" in warning:
                if "weld_temperature" in warning:
                    ...
                    if click.confirm(f"Increase temperature to {suggested_temp}°C?"):
                        section, param = self._find_config_parameter(config, "weld_temperature")
                        if section and param:
                            config[section][param] = suggested_temp
                            modified = True
                            self.fixes_applied.append(f"Increased {section}.{param} to {suggested_temp}°C")
                    ...
                    if click.confirm(f"Increase weld time to {suggested_time}s?"):
                        section, param = self._find_config_parameter(config, "weld_time")
                        if section and param:
                            config[section][param] = suggested_time
                            modified = True
                            self.fixes_applied.append(f"Increased {section}.{param} to {suggested_time}s")
    ...
    def _check_missing_sections(self, config: Dict) -> List[str]:
        ...
        required_sections = [
            ...
            "normal_welds", "light_welds", "animation", "output"
        ]
    ...
    def _add_missing_sections(self, config_path: Path, config: Dict, missing_sections: List[str]) -> None:
    ...
    def _find_config_parameter(self, config: Dict, param_name: str) -> Tuple[Optional[str], Optional[str]]:
...
def setup_wizard() -> None:
    ...
    if click.confirm("Do you want to configure printer connectivity?"):
        ...
        click.echo("⏭️  Skipping printer setup (you can run 'microweldr init-secrets' later)")
