...
class HealthChecker:
    ...
    def run_all_checks(self, secrets_path: Optional[str] = None) -> Dict[str, Any]:
        ...
        if secrets_path and Path(secrets_path).exists():
            ...
            self.checks['printer'] = {'status': 'skipped', 'message': 'No secrets file provided'}
            self.checks['secrets'] = {'status': 'skipped', 'message': 'No secrets file provided'}
    ...
    def _check_python_version(self) -> Dict[str, Any]:
        ...
        if version_info < (3, 8):
            ...
            self.warnings.append(f"Python {version_str} is supported but newer versions recommended")
            return {
                'status': 'warning',
                'version': version_str,
    ...
    def _check_filesystem_access(self) -> Dict[str, Any]:
        ...
        test_paths = [
            ...
            Path('/tmp') if platform.system() != 'Windows' else Path.cwd() / 'temp'  # Temp directory
        ]
        ...
        for config_file in config_files:
            ...
            if config_path.exists():
                try:
                    ...
                    if missing_sections:
                        config_issues.append(f"{config_file}: missing sections {missing_sections}")
    ...
    def _check_validation_tools(self) -> Dict[str, Any]:
        ...
        if len(available_tools) == 0:
            ...
            self.warnings.extend([f"Missing validation tool: {tool}" for tool in missing_tools])
            return {
                'status': 'warning',
                'available': available_tools,
                'missing': missing_tools,
    ...
    def _check_secrets_security(self, secrets_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if errors:
                ...
                self.warnings.extend([f"Secrets security: {warning}" for warning in warnings])
                return {
                    'status': 'warning',
                    'warnings': warnings,
                    'errors': [],
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if memory_check.get('status') in ['warning', 'error']:
            ...
            recommendations.append("Consider processing smaller SVG files or using caching")
        ...
        if disk_check.get('status') in ['warning', 'error']:
            ...
            recommendations.append("Consider using a different working directory with more space")
        
        # Dependencies recommendations
        deps_check = self.checks.get('dependencies', {})
        if deps_check.get('missing_optional'):
            recommendations.append("Install optional packages for enhanced functionality:")
        ...
        if config_check.get('status') == 'warning':
            recommendations.append("Run 'microweldr validate config.toml' to fix configuration issues")
        
        # Printer recommendations
        printer_check = self.checks.get('printer', {})
        if printer_check.get('status') == 'error':
            recommendations.append("Check printer network connection and PrusaLink settings")
            recommendations.append("Verify secrets.toml contains correct printer credentials")
        elif printer_check.get('status') == 'skipped':
            recommendations.append("Run 'microweldr init-secrets' to set up printer connectivity")
        ...
        if secrets_check.get('status') in ['warning', 'error']:
            ...
            recommendations.append("Ensure secrets.toml has restricted file permissions (600)")
...
def quick_health_check() -> Tuple[str, List[str]]:
    ...
    for check_name, result in critical_checks.items():
        if result.get('status') == 'error':
            has_errors = True
            critical_issues.append(f"{check_name}: {result.get('message', 'Unknown error')}")
...
def generate_health_report(output_path: Optional[str] = None) -> str:
    ...
    if health_status['errors']:
        report_lines.extend([
            "",
            "Errors:",
            *[f"  • {error}" for error in health_status['errors']]
        ])
