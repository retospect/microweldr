...
class Colors:
    ...
    PIPETTE_ALIASES = {
        "magenta", "pink", "fuchsia", "#ff00ff", "#f0f", "#ff69b4", "#ffc0cb",
...
class WarningMessages:
    ...
    EXPERIMENTAL_MODE = "⚠️  WARNING: Layed back mode is EXPERIMENTAL and may not work properly!"
...
class LogMessages:
    ...
    VALIDATION_FAILED = "Validation failed: {file} - {errors} errors, {warnings} warnings"
...
def get_operating_mode_enum(mode_str: str) -> OperatingMode:
    ...
    try:
        ...
        raise ValueError(f"Invalid operating mode: {mode_str}. Must be one of: {', '.join(valid_modes)}")
