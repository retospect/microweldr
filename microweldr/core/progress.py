...
class SimpleProgressReporter:
    ...
    def finish(self) -> None:
        """Finish progress reporting."""
        elapsed = time.time() - self.start_time
        logger.info(f"{self.description} completed: {self.total} items in {elapsed:.2f}s")
...
class BatchProgressReporter:
    ...
    def start_batch(self, batch_size: int, batch_description: str = None) -> ProgressReporter:
        """Start a new batch.
    ...
    def finish(self) -> None:
        ...
        elapsed = time.time() - self.start_time
        logger.info(f"{self.description} completed: {self.batches} batches in {elapsed:.2f}s")
...
def with_progress(
    ...
    def decorator(func: Callable) -> Callable:
        def wrapper(*args, **kwargs):
            ...
            if total > 1 and (show_progress is True or (show_progress is None and total > 10)):
