...
def temp_off(
    ...
    try:
        ...
        if cooldown_temp < 20 or cooldown_temp > 60:
            click.echo(f"❌ Invalid cooldown temperature: {cooldown_temp}°C (must be 20-60°C)", err=True)
            sys.exit(1)
        ...
        try:
            ...
            if status.state == "Printing":
                ...
                if not force and not click.confirm("Continue anyway? This may affect the print!"):
                    click.echo("❌ Temperature control cancelled")
                    sys.exit(0)
            ...
            if turn_off_bed:
                gcode_commands.append(f"{GCodeCommands.M140} S{cooldown_temp} ; Set bed temperature")
                click.echo(f"🛏️  Setting bed temperature to {cooldown_temp}°C")
            
            if turn_off_nozzle:
                gcode_commands.append(f"{GCodeCommands.M104} S{cooldown_temp} ; Set nozzle temperature")
                click.echo(f"🔥 Setting nozzle temperature to {cooldown_temp}°C")
            
            if turn_off_chamber:
                gcode_commands.append(f"{GCodeCommands.M141} S0 ; Turn off chamber heating")
            ...
            try:
                ...
                result = client.upload_gcode(temp_gcode_path, "microweldr_cooldown.gcode")
                if result['success']:
                    ...
                    if client.start_print("microweldr_cooldown.gcode"):
                        ...
                        for i in range(5):
                            ...
                            click.echo(f"   Bed: {current_status.bed_actual}°C, Nozzle: {current_status.nozzle_actual}°C")
                        
                        click.echo("✅ Temperature cooldown initiated successfully")
                    else:
                        click.echo("⚠️  G-code uploaded but failed to start", err=True)
                else:
                    click.echo(f"❌ Failed to upload G-code: {result.get('error', 'Unknown error')}", err=True)
                    sys.exit(1)
...
def temp_on(
    ...
    try:
        ...
        click.echo(f"Targets: Bed → {bed_temp}°C, Nozzle → {nozzle_temp}°C, Chamber → {chamber_temp}°C")
        ...
        try:
            ...
            gcode_commands.append(f"{GCodeCommands.M140} S{bed_temp} ; Set bed temperature")
            gcode_commands.append(f"{GCodeCommands.M104} S{nozzle_temp} ; Set nozzle temperature")
            
            # Chamber heating (if supported)
            use_chamber = main_config.get(ConfigSections.TEMPERATURES, ConfigKeys.USE_CHAMBER_HEATING, True)
            if use_chamber and chamber_temp > 0:
                gcode_commands.append(f"{GCodeCommands.M141} S{chamber_temp} ; Set chamber temperature")
            
            if wait:
                gcode_commands.append("")
                gcode_commands.append("; Wait for temperatures")
                gcode_commands.append(f"{GCodeCommands.M190} S{bed_temp} ; Wait for bed temperature")
                gcode_commands.append(f"{GCodeCommands.M109} S{nozzle_temp} ; Wait for nozzle temperature")
                if use_chamber and chamber_temp > 0:
                    gcode_commands.append(f"{GCodeCommands.M191} S{chamber_temp} ; Wait for chamber temperature")
                    ...
                    if client.start_print("microweldr_heatup.gcode"):
                        ...
                        if wait:
                            ...
                            while time.time() - start_time < 300:  # 5 minute timeout
                                time.sleep(5)
                                current_status = client.get_status()
                                click.echo(f"   Bed: {current_status.bed_actual}°C/{bed_temp}°C, "
                                         f"Nozzle: {current_status.nozzle_actual}°C/{nozzle_temp}°C")
                                
                                # Check if temperatures reached
                                bed_ready = abs(current_status.bed_actual - bed_temp) <= 2
                                nozzle_ready = abs(current_status.nozzle_actual - nozzle_temp) <= 2
                                
                                if bed_ready and nozzle_ready:
                                    click.echo("✅ Target temperatures reached!")
                                    break
                            else:
                                click.echo("⚠️  Timeout waiting for temperatures", err=True)
                        else:
