...
from microweldr.api import (
    MicroWeldr, WeldJob, ValidationResult,
    PrinterConnection, PrinterStatus,
    WorkflowManager, WorkflowStep,
    ValidationSuite, SystemMonitor, HealthStatus
)
...
def example_basic_usage():
    ...
    with welder.create_job("examples/calibration_test.svg") as job:
        ...
        if validation.is_valid:
            ...
            print(f"Job stats: {stats['total_points']} points in {stats['total_paths']} paths")
            ...
            with welder.create_job("examples/calibration_test.svg") as job:
                ...
                if result['success']:
                    ...
                    if printer.start_print(result['filename']):
                        ...
                        if progress:
                            print(f"Print progress: {progress['progress_percent']:.1f}%")
        else:
...
def example_workflow_management():
    ...
    workflow.add_animation_generation_step("output/workflow_animation.svg", required=False)
...
def example_system_monitoring():
    ...
    print(f"Platform: {sys_info['platform']['system']} {sys_info['platform']['release']}")
...
def example_advanced_workflow():
    ...
    workflow.add_gcode_generation_step(
        "output/production.gcode",
        name="generate_gcode"
    )
    ...
    if secrets_path.exists():
        try:
            ...
            workflow.add_printer_submission_step(
                printer,
                name="submit_to_printer",
                auto_start=False,
                wait_for_ready=True
            )
            
            # Add monitoring step (optional)
            workflow.add_print_monitoring_step(
                printer,
                name="monitor_print"
            )
...
def example_configuration_management():
    ...
    welder.update_config({
        'temperatures': {
            'bed_temperature': 65  # Increase bed temperature
        },
        'normal_welds': {
            'weld_time': 0.15  # Increase weld time
        }
    })
